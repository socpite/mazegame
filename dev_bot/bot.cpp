...
namespace example{
    ...
    void set_random_walls(Game &game) {
        vector<pair<int, int>> q;
        q.push_back(game.start_pos);
        ...
        while(!q.empty()) {
            ...
            shuffle(q.begin(), q.end(), example::rng);
...
// game.item_board is a 2D array of size n x m, where each element is the item name in the cell (i, j). Items should be one of the item types in game.item_types
// You can change walls, items, starting position and ending position.

// RULES
/*
    - Starting position must differ from ennding position
    - Starting position should not contain an item
    - All cells must be reachable from the starting position
    - Additional rules can be accessed via game.rule. Please refer to the cppstubs.h file for what additional rules are available.
*/

Game create_maze(Game game) {
    cerr << "Creating maze..." << endl;
    example::reset();
    ...
    example::set_random_walls(game);

    example::set_random_items(game);
